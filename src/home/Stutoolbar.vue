    ...
    <v-toolbar-items class="hidden-sm-and-down toolbar-right">
      ...
      <v-menu open-on-hover offset-y top transition="slide-y-transition">
        <v-btn slot="activator" flat>
          <v-avatar size="36"><img src="/src/assets/xnick.jpg" alt="avatar"></v-avatar>
          <v-icon class="ml-1">keyboard_arrow_down</v-icon>
        </v-btn>
        <v-list>
          <v-list-tile v-for="(item, index) in items" :key="index">
            <router-link :to="{ name: 'stuown', params: {opt:item.opt}}"><v-list-tile-title class="ml-3">{{ item.title }}</v-list-tile-title></router-link> 
          </v-list-tile>
        </v-list>
      </v-menu>
      </router-link>
      <router-link :to="{ name: 'stuown', params: {opt:'stumsg'}}">
