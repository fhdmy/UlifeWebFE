<template>
    <v-content style="background: #f3f4f5;" v-scroll="onScroll">
      <div class="elevation-1 white home-toolbar-wrapper" :style="{'opacity':toolbaropacity,'display':display}">
        <Org-toolbar :avatar="opt.avatar" :name="opt.name"></Org-toolbar>
      </div>
      ...
      <div class="elevation-1 white" :class="{'isfixed':fixed,'owntoolbar-wrapper':true}">
        <div class="middle-wrapper">
          ...
          <Preview-toolbar :org="opt.name" :launchdate="opt.launchdate" :isfinished="opt.isfinished" :stars="opt.stars" :fixed="fixed" :title="opt.title"></Preview-toolbar>
        </div>
      </div>
      <div v-if="fixed" style="height:70px;"></div>
      <div class="main-wrapper">
         <Preview-body :introduction="opt.introduction" :date="opt.date" :time="opt.time" :place="opt.place" :type="opt.type" :interest="opt.interest" :lists="opt.lists" :customlists="customlists"></Preview-body>
