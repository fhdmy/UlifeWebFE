<template>
  <v-toolbar color="white elevation-0">
    ...
    <v-toolbar-items class="hidden-sm-and-down toolbar-right">
      <router-link :to="{ name: 'student_own', params: {opt:'student_signup'}}">
      <v-menu open-on-hover offset-y top transition="slide-y-transition">
        ...
        <v-list>
          <v-list-tile v-for="(item, index) in items" :key="index">
            <router-link :to="{ name: 'student_own', params: {opt:item.opt}}"><v-list-tile-title class="ml-3">{{ item.title }}</v-list-tile-title></router-link> 
          </v-list-tile>
        </v-list>
      </v-menu>
      </router-link>
      <router-link :to="{ name: 'student_own', params: {opt:'student_msg'}}">
